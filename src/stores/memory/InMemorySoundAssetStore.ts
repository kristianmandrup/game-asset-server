...
export class InMemorySoundAssetStore extends InMemoryAssetStore<Sound> {
   ...
   */
  createAsset = async (sound: Sound): Promise<Sound> => {
    try {
      SoundSchema.parse(sound); // Validate the sound asset
      return super.createAsset(sound);
    } catch (error: any) {
   ...
   */
  getAssets = async (query?: Partial<Sound>): Promise<Sound[]> => {
   ...
   */
  getAssetById = async (id: string): Promise<Sound | undefined> => {
   ...
   */
  updateAsset = async (id: string, sound: Sound): Promise<Sound> => {
    try {
      SoundSchema.parse(sound); // Validate the sound asset
      return super.updateAsset(id, sound);
    } catch (error: any) {
